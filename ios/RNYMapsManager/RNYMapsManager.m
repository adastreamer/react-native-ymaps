...
@interface RNYMapsManager() <MKMapViewDelegate>
...
RCT_CUSTOM_VIEW_PROPERTY(region, MKCoordinateRegion, RNYMaps)
{
...
- (void)mapView:(RNYMaps *)mapView didSelectAnnotationView:(MKAnnotationView *)view
{
...
- (void)mapView:(RNYMaps *)mapView annotationView:(MKAnnotationView *)view calloutAccessoryControlTapped:(UIControl *)control
{
    if (mapView.onPress) {
...
- (void)mapView:(RNYMaps *)mapView didUpdateUserLocation:(MKUserLocation *)location
{
    if (mapView.followUserLocation) {
        MKCoordinateRegion region;
        region.span.latitudeDelta = RNYMapsDefaultSpan;
        region.span.longitudeDelta = RNYMapsDefaultSpan;
...
- (void)mapView:(RNYMaps *)mapView regionWillChangeAnimated:(__unused BOOL)animated
{
    [self _regionChanged:mapView];
    
    mapView.regionChangeObserveTimer = [NSTimer timerWithTimeInterval:RNYMapsRegionChangeObserveInterval
                                                               target:self
                                                             selector:@selector(_onTick:)
                                                             userInfo:@{ RNYMapsViewKey: mapView }
                                                              repeats:YES];
    
    [[NSRunLoop mainRunLoop] addTimer:mapView.regionChangeObserveTimer forMode:NSRunLoopCommonModes];
}

- (void)mapView:(RNYMaps *)mapView regionDidChangeAnimated:(__unused BOOL)animated
{
...
- (void)mapViewWillStartRenderingMap:(RNYMaps *)mapView
{
...
{
    [self _regionChanged:timer.userInfo[RNYMapsViewKey]];
}

- (void)_regionChanged:(RNYMaps *)mapView
{
    BOOL needZoom = NO;
    ...
    if (mapView.maxDelta > FLT_EPSILON && region.span.longitudeDelta > mapView.maxDelta) {
        needZoom = YES;
        newLongitudeDelta = mapView.maxDelta * (1 - RNYMapsZoomBoundBuffer);
    } else if (mapView.minDelta > FLT_EPSILON && region.span.longitudeDelta < mapView.minDelta) {
        needZoom = YES;
        newLongitudeDelta = mapView.minDelta * (1 + RNYMapsZoomBoundBuffer);
    }
    if (needZoom) {
...
- (void)_emitRegionChangeEvent:(RNYMaps *)mapView continuous:(BOOL)continuous
{
    if (mapView.onChange) {
        MKCoordinateRegion region = mapView.region;
        if (!CLLocationCoordinate2DIsValid(region.center)) {
